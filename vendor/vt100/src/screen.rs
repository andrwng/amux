...
impl Screen {
    ...
    // CSI r
    /// The active scroll region as 0-based inclusive rows `(top, bottom)`. Added for amux so a
    /// reattach snapshot can restore the region; upstream vt100 exposes no getter, so a rebuilt
    /// client parser would default to the whole screen and scroll differently from the daemon.
    #[must_use]
