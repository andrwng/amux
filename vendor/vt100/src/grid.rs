...
impl Grid {
    ...
    /// The current scroll region as 0-based inclusive rows `(top, bottom)`. Added for amux: a
    /// reattaching client rebuilds its parser from a snapshot, and without the region it scrolls the
    /// whole screen where the app scrolls a sub-range. vt100 upstream exposes no getter.
